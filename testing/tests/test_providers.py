...
def test_provider_spec(provider_spec: ProviderSpec):
    ...
    ), "provider_spec must be a ProviderSpec instance"
    assert issubclass(
        provider_spec.config_cls, llm.ProviderConfig
    ), "config_cls must be a ProviderConfig subclass"
    assert issubclass(
        provider_spec.chat_cls, llm.ProviderChat
    ), "chat_cls must be a ProviderChat subclass"
...
def test_provider_config_registration(provider_config_cls: type[llm.ProviderConfig]):
    assert issubclass(
        provider_config_cls, llm.ProviderConfig
    ), "expected_class must be a ProviderConfig subclass"
    ...
    assert (
        config_cls is provider_config_cls
...
def test_from_generic_config(provider_config_cls: type[llm.ProviderConfig]):
    ...
    assert isinstance(
        provider_config, llm.ProviderConfig
...
def test_provider_chat_instantiation(provider_spec: ProviderSpec):
    ...
    for chat in [provider_chat_no_config, provider_chat_with_config]:
        assert hasattr(
            chat, "ask"
        ), "ProviderChat instance does not have an 'ask' method"
        assert callable(
            getattr(chat, "ask", None)
        ), "'ask' method of ProviderChat instance is not callable"
        assert hasattr(
            chat, "close"
        ), "ProviderChat instance does not have a 'close' method"
        assert callable(
            getattr(chat, "close", None)
        ), "'close' method of ProviderChat instance is not callable"
        assert hasattr(
            chat, "id"
        ), "ProviderChat instance does not have an 'id' attribute"
        assert hasattr(
            chat, "config"
        ), "ProviderChat instance does not have a 'config' attribute"
