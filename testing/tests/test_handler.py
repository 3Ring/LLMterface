...
def test_instantiate_handler():
    ...
    ), "Handler should be an instance of LLMterface"
...
def test_ask_with_temp_chat():
    ...
    question = llm.Question[str](
        question="What is the airspeed velocity of an unladen swallow?", config=config
    )
    with handler.temp_chat(provider=FakeProviderConfig.PROVIDER) as chat:
        response = chat.ask(question)
        assert isinstance(response, str), "Response should be a string"
        assert (
            "African or European swallow" in response
        ), "Response text should contain expected answer"
